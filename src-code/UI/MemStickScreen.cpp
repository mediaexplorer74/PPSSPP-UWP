...
static void AddExplanation(UI::ViewGroup *viewGroup, MemStickScreen::Choice choice, UI::View *extraView = nullptr) {
	...
	case MemStickScreen::CHOICE_BROWSE_FOLDER:
		...
		holder->Add(new TextView(iz->T("EasyUSBAccess", "Easy USB access"), flags, false))->SetBullet(true);
#else
		holder->Add(new TextView(iz->T("Homebrew", "Less compatibility with Homebrew games"), flags, false))->SetBullet(true);
#endif
		break;
...
#endif
#else
		holder->Add(new TextView(iz->T("Homebrew", "Better compatibility with Homebrew games"), flags, false))->SetBullet(true);
#endif
		break;
